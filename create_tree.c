...
/*   Updated: 2023/12/01 22:24:46 by aminakov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	create_tree(t_tree **tree, char *str, t_sgm sgm, t_data *data)
{
	...
	*tree = (t_tree *)malloc(sizeof(t_tree));
	...
	prefill_node(*tree, str, sgm, data);
...
int	do_if_sep(t_tree *tree, int sep_pos, t_node sep)
{
	...
	create_tree(&tree->left_child, tree->cmd, \
			cr_sgm(tree->beg, sep_pos), tree->data);
	create_tree(&tree->right_child, tree->cmd, \
		cr_sgm(sep_pos + len_sep(sep), tree->end), tree->data);
	tree->beg = sep_pos;
...
int	doifbrackets_in_create_tree(t_tree *tree, t_sgm brackets)
{
	...
	create_tree(&tree->right_child, tree->cmd, \
		cr_sgm(brackets.b + 1, brackets.e), tree->data);
	tree->beg = brackets.b;
